...
**Teal usage rule:** `--teal-deep` is the small/mono text choice (labels, eyebrows, links on cream). `--teal` is the fill, the hover, the selection, the large shape. Choosing between them inline is a bug; follow the token rule instead.
...
| Meta / eyebrow / index (`MetaLabel`, `SectionNumber`) | JetBrains Mono | 12px | 500 (meta) · 600 (section) | meta: 0.12em, uppercase; section: 0.04em, sentence-case |
| Inline code | JetBrains Mono | 0.92em | 400 | quiet tint, no box |

**Locked readability rule (do not regress):** no informational text ships below 11px, and no informational text ships in `--ink-muted` (≈3.3:1 on cream, fails AA). `--ink-muted` stays for decorative/background elements only. `MetaLabel` default is 12px / `--ink-light` (≈7:1, passes AA/AAA). The `size="section"` variant (12px / 600 / `--ink`, sentence-case) is for structural headings — intake form sections and grouped headers. Sentence case, not uppercase, for multi-word section headings: long uppercase strings are the hardest thing for dyslexic/ADHD readers to scan (ND audience = the product).
...
Mono label: 12px / 500, default `--ink-light`, uppercase, 0.12em. `size="section"` switches to sentence-case structural headings: 12px / 600 / `--ink`, 0.04em, `marginBottom` default 14. Use `section` for intake form section headings and grouped headers; keep `meta` for true eyebrows and per-item labels.

### HTML export (`lib/export-html.ts`)
Every output artifact offers a self-contained HTML download alongside the markdown: `Download page (HTML)`. The export serializes the rendered artifact DOM (inline styles already reference the CSS variables, so the export embeds the `:root` token block), inlines a `@media print` block (cream → white, clean PDF), uses a system font stack (no web fonts — must open offline), strips interactivity, and carries the header line "Generated by NeuroDiv OS, in your own browser. Nothing was sent to any server." Filenames are date-stamped and lowercase: `nd-profile-2026-08-14.html`, `nd-process-2026-08-14.html`. Never read as a clinical report: no scores, no results, no percentile framing.
...
- **Privacy promise is a core surface, stated on every entry point:** nothing the user types is collected, stored, or analyzed; no account, no cookies, no tracking; data lives only in the browser's local storage (keys: `nd-profile`, `nd-process-designer`, `nd-process-designer-artifacts`, `nd-process-designer-current`). Front door carries the full block; the Context Builder intro and every downloaded artifact repeat one line: "Nothing you type here leaves this browser" / "Generated in your own browser. Nothing was sent to any server." The promise must stay literally true — fonts are self-hosted, no third-party requests on load.
